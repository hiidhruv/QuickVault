 ...
 * @returns The masked URL using img.intercomm.in
 */
export function maskCatboxUrl(url: string): string {
  if (!url) return url

  // If it's already using our domain, return as is
  if (url.includes("img.intercomm.in")) {
    return url
  }
  ...
  if (url.includes("catbox.moe")) {
    ...
    return `https://img.intercomm.in/${filename}`
  }
...
export function unmaskUrl(url: string, originalUrl?: string): string {
  ...
  // If we have the original URL and the masked URL is using our domain, return the original
  if (originalUrl && url.includes("img.intercomm.in")) {
    return originalUrl
  }

  // If it's using our domain but we don't have the original, try to reconstruct it
  if (url.includes("img.intercomm.in")) {
    const filename = getFilenameFromUrl(url)
    return `https://files.catbox.moe/${filename}`
  }

  // Return the URL as is if it's not using our domain
  return url
}
