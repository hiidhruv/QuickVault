  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {images.map((image) => {
        ...
        const imageUrl = `${window.location.origin}/i/${image.id}`
        
        return (
          <div key={image.id} className="group relative overflow-hidden border bg-background transition-colors hover:bg-accent">
            <Link
              href={`/i/${image.id}`}
              className="block"
            >
